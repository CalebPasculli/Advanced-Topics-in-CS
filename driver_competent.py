...
# addition
from netflow_ksp import greedy_ksp_nodes
...
def parse_args():
    parser = ArgumentParser(
        prog="ShotHound", prefix_chars="-/", add_help=False,
        description='Finding practical paths in BloodHound'
    )
    ...
    parser.add_argument('--k_weighting', type=str, default='uniform')   # 'uniform' or 'exp'
    parser.add_argument('--lambda_len', type=float, default=0.0)        # only if k_weighting=exp
    parser.add_argument('--k_solver', type=str, default='CBC')          # CBC/GUROBI/SCIP
...
args = {
    "fn": cli.fn,
    "budget": cli.budget,
    "start_node_number": cli.start,
    "algo": cli.algo,
    "blockable_p": -1,   # -1 AAAI-23 policy; -2 => only computers blockable
    "double_edge_p": 0,
    "multi_block_p": 1,
    "cross_path_p": 0,
    "no_one_hop": True,
    ...
    # k-SP additions
    "k": cli.k,
    "k_weighting": cli.k_weighting,
    "lambda_len": cli.lambda_len,
    "k_solver": cli.k_solver,
}
...
if __name__ == "__main__":
    ...
    for seed in range(number_of_runs):
        ...
        # DO NOT unpack args.values(); read by key to avoid order issues
        fn = args["fn"]
        budget = args["budget"]
        start_node_number = args["start_node_number"]
        algo = args["algo"]
        blockable_p = args["blockable_p"]
        double_edge_p = args["double_edge_p"]
        multi_block_p = args["multi_block_p"]
        cross_path_p = args["cross_path_p"]
        no_one_hop = args["no_one_hop"]
        seed_val = args["seed"]

        CG = flow_graph_setup(fn, seed_val, start_node_number, blockable_p,
                            double_edge_p, multi_block_p, budget, cross_path_p, no_one_hop)
        ...
        if algo == "greedy_competent":
            ...
            strat = greedy_ksp_nodes(graph_original, k=args["k"])
            score = mixed_evaluate_helper(graph_original, strat)
            score_list.append(score)
            time_list.append(0)
